...
This creates a sub-account, `status_message.cross_contract`, and transfers 10 $NEAR to it (the long number above is a 1e25, which is the number of yoctoNEAR to transfer; it's larger than JS's JSON number max size of 2^53 and so it must serialized as a string).

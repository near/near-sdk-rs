...
fn qc_test_total_supply(initial_supply: Balance) -> bool  {
    ...
    assert_eq!(root.get_balance(&mut runtime, root.account_id().into()), deposit_amount);
    return true;
}
...
fn qc_test_transfer(initial_supply: Balance) -> bool {
  ...
  let total_supply = root.get_total_supply(&mut runtime);
  assert_eq!(total_supply, deposit_amount);
  assert_eq!(root.get_balance(&mut runtime, root.account_id().into()), deposit_amount - transfer_amout);
  assert_eq!(root.get_balance(&mut runtime, bob.account_id().into()), transfer_amout);
  return true;
}
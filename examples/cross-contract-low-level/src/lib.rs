...
use near_sdk::{env, near_bindgen, require_eq, AccountId, Gas, PromiseResult};
...
impl CrossContract {
    ...
    pub fn merge(&self) -> Vec<u8> {
        require_eq!(env::current_account_id(), env::predecessor_account_id());
        require_eq!(env::promise_results_count(), 2);

...
mod tests {
    ...
    async fn simulate_enum_all_tokens() -> anyhow::Result<()> {
        ...
        .await?;
        let token_3: Token =
            helper_mint(&mt, alice.id().clone(), 5u128, "title3".to_string(), "desc3".to_string())
                .await?;

        // Get all tokens
        let res: Vec<Token> = mt
            .call("mt_tokens")
        ...
        let res: Vec<Token> =
            mt.call("mt_tokens").args_json((Some(U128(2)), Some(1u64))).view().await?.json()?;
    ...
    async fn simulate_enum_tokens_for_owner() -> anyhow::Result<()> {
        ...
        .await?;
        helper_mint(&mt, defi.id().clone(), 20_000u128, "title2".to_string(), "desc2".to_string())
            .await?;
        let token_3: Token =
            helper_mint(&mt, alice.id().clone(), 5u128, "title3".to_string(), "desc3".to_string())
                .await?;
        let token_4: Token = helper_mint(
            &mt,
            defi.id().clone(),
            20_000u128,
            "title4".to_string(),
        ...
        .await?;
        let token_5: Token =
            helper_mint(&mt, alice.id().clone(), 5u128, "title5".to_string(), "desc5".to_string())
                .await?;

    ...
    T: BorshSerialize + BorshDeserialize,
{
    ...
    fn nth(&mut self, n: usize) -> Option<Self::Item> {
        ...
        Some(self.vec.get(idx).unwrap_or_else(|| env::panic_err(standard_errors::IndexOutOfBounds{}.into())))
    }
}
    ...
    T: BorshSerialize + BorshDeserialize,
{
    ...
    fn nth(&mut self, n: usize) -> Option<Self::Item> {
        let idx = self.range.nth(n)?;
        Some(self.get_mut(idx).unwrap_or_else(|| env::panic_err(standard_errors::IndexOutOfBounds{}.into())))
    }
}

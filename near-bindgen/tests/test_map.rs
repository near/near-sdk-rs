...
fn test_pop() {
    ...
    for i in 1..5 {
        let result = execute(&code, b"map_remove", format!("{{\"key\": {} }}", i).as_bytes(), &[], &mut ext, &config, &context).unwrap();
...
fn test_insert() {
    ...
    let result = execute(&code, b"map_insert", b"{\"key\": 4, \"value\": 200}", &[], &mut ext, &config, &context).unwrap();
    check_result!(result, 14);
    let result = execute(&code, b"map_to_vec", &[], &[], &mut ext, &config, &context).unwrap();
    check_result!(result, [[0, 10], [1, 11], [2, 12], [3, 13], [4, 200]]);
    let result = execute(&code, b"map_insert", b"{\"key\": 5, \"value\": 100}", &[], &mut ext, &config, &context).unwrap();
    check_result_str!(result, "null");

    execute(&code, b"map_clear", &[], &[], &mut ext, &config, &context).unwrap();
    let result = execute(&code, b"map_insert", b"{\"key\": 0, \"value\": 0}", &[], &mut ext, &config, &context).unwrap();
    check_result_str!(result, "null");
    let result = execute(&code, b"map_to_vec", &[], &[], &mut ext, &config, &context).unwrap();
